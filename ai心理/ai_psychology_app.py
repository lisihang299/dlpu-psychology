...
with tab7:
    ...
    # ==================== 关键修改：强制绑定真实账号，永不随机 ====================
    # 👇👇 替换这里！把 YOUR_REAL_USER_FIELD 改成你系统里的真实字段名（比如username/visitor）
    REAL_USER_FIELD = "YOUR_REAL_USER_FIELD"  # 示例：改成 "visitor" 或 "username"
    
    # 直接获取系统真实账号，没有兜底、不随机
    if REAL_USER_FIELD in st.session_state and st.session_state[REAL_USER_FIELD]:
        CURRENT_USER = str(st.session_state[REAL_USER_FIELD])  # 确保是字符串
    else:
        st.error("⚠️ 未检测到登录账号，请先登录！")
        st.stop()  # 没有账号直接停止，避免生成随机ID
    
    # 管理员判断（按需修改：比如账号是"admin"则为管理员）
    ...
    # 会话状态初始化（仅保留临时数据，去掉user_unique_id）
    ...
    if IS_ADMIN:
        st.markdown(f'<p class="admin-tip">🔧 管理员调试：当前账号 {CURRENT_USER} | 帖子数 {len(shared_data["posts"])}</p>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        # 发布按钮（关键修改：publisher_id 绑定 CURRENT_USER）
            ...
            for post_idx, post in enumerate(posts):
                ...
                with post_head2:
                    st.markdown(f'<span style="color:#a89988; font-size:10px; text-align:right; display:block;">{post["create_time"]}</span>',
                                unsafe_allow_html=True)
                # 删除权限（关键修改：比对 CURRENT_USER）
                with post_head3:
                    can_delete_post = IS_ADMIN or (post.get("publisher_id") == CURRENT_USER)
                    if can_delete_post:
                ...
                if post["comments"] and len(post["comments"]) > 0:
                    ...
                    for c_idx, c in enumerate(post["comments"]):
                        ...
                        with cmt_col2:
                            # 评论删除权限（比对真实账号）
                            can_delete_cmt = IS_ADMIN or (c.get("commenter_id") == CURRENT_USER)
                            if can_delete_cmt:
                                st.markdown('<div class="admin-delete-btn">', unsafe_allow_html=True)
