...
with tool6:
        ...
        # 新增：初始化用户唯一标识（基于登录态的username，若无则用会话ID兜底）
        if "user_identifier" not in st.session_state:
            # 优先使用登录用户的username，无则用streamlit内置会话ID
            st.session_state.user_identifier = st.session_state.get("user_info", {}).get("username", st.session_id)
